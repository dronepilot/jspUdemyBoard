...
<center>
<h2>전체 게시글 보기</h2>
<!-- 게시글 보기에 카운터링을 설정하기위한 변수들 선언 -->
<%

	// 화면에 보여질 게시글의 개수를 지정
	int pageSize = 10;

	// 현재 카운터를 클릭한 번호값을 읽어옮
	String pageNum = request.getParameter("pageNum");
	
	// 만약 처음 boardlist.page를 클릭하거나 수정 삭제 등 다른 게시글에서 이페이지로 넘어오면 pageNum값이 없기에
	// null 처리를 해줌
	if(pageNum == null) {
		pageNum = "1";
	}
...
<%
	if(count > 0) {
		...
		int pageCount = count / pageSize + (count%pageSize == 0? 0 : 1);	// 카운터링 숫자를 얼마까지 보여줄 것인지 결정 ex) 71개면 8페이지
